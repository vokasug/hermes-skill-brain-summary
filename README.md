...
1. **Голая ссылка на YouTube** (или «о чём видео» + YouTube-ссылка) — скачивает аудио, транскрибирует локально и возвращает плотный пересказ; транскрипт и подробный пересказ архивируются в `~/gdrive/Brain/`.
2. **«Добавь в мозг» / «добавь в brain»** + что добавить — архивация любого контента в базу знаний `~/gdrive/Brain/`: медиа (тот же пайплайн yt-dlp + mlx-whisper), статьи и страницы (tavily extract), темы без ссылки (поиск → лучший источник), файлы (аудио/видео → транскрибация; документы/html → трансформация в MD, для html — `scripts/html2md.py`), готовый текст.

Голая ссылка НЕ на YouTube скилл не запускает — только через явное «добавь в мозг/brain».
...
- **Ссылка — один раз, в блоке метаданных в конце и в `code`-формате** — любая кликабельная ссылка в сообщении даёт embed-превью сайта; первая строка ответа — название видео, автор, длительность, язык
...
- **Архивация в базу знаний** — готовый MD транскрипта копируется в предметную папку `~/gdrive/Brain/` (выбор по правилам `Brain/AGENTS.md`), рядом сохраняется подробный пересказ `YYYY-MM-DD_<имя>_sum.md` (до 16000 символов, та же структура «Главная мысль / Ключевые тезисы / Вывод» + блок метаданных); в ответе пользователю — блок с метаданными и названием папки Brain
...
Скриптовая часть — только `scripts/html2md.py` (конвертер HTML→MD для не-медиа ветки). Пути `~/result-yt-dlp` / `~/result-mlx-whisper` и расположение моделей наследуются от скиллов-компонентов; на своём Mac настройте их по README тех репозиториев. Внешние API и ключи для коррекции не нужны — советчиком выступает основная модель агента.
...
└── scripts/
    └── html2md.py  # HTML-отчёт → MD полного содержания (stdlib), для не-медиа ветки
```
