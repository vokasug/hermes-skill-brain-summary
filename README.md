...
- **Субтитры как «второе мнение»** — всегда пытается скачать субтитры контента (`--write-subs --write-auto-subs`); если есть, они идут в LLM-коррекцию mlx-whisper как второй ASR-текст + источник авто-терминов (чинит ослышки имён/терминов; числа защищены гейтом в коде)
