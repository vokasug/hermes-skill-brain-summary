...
- `brain-vector` (локальный скилл, без отдельного репозитория) — векторная база поверх `*_sum.md`: семантический поиск по вопросам «мозг …» с фильтрами по папке/автору/дате. Пополнение — кроном каждые 4 часа (инкрементально), оптимизация — ежедневно в 06:00. Новый sum-файл из этого скилла попадёт в базу ближайшим прогоном.
