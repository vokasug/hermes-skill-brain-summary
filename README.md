...
Скилл [Hermes Agent](https://hermes-agent.nousresearch.com/docs) с тремя триггерами:

1. **«Добавь в мозг» / «добавь в brain» / «+мозг»** + что добавить — архивация любого контента в базу знаний `~/gdrive/Brain/`: медиа (тот же пайплайн yt-dlp + mlx-whisper), статьи и страницы (tavily extract), темы без ссылки (поиск → лучший источник), файлы (аудио/видео → транскрибация; документы/html → трансформация в MD, для html — `scripts/html2md.py`), готовый текст.
2. **Ссылка на YouTube без команды** (или «о чём видео» + YouTube-ссылка) — скачивает аудио, транскрибирует локально и возвращает плотный пересказ; транскрипт и подробный пересказ архивируются в `~/gdrive/Brain/`.

Ссылка НЕ на YouTube без команды скилл не запускает — только через явное «добавь в мозг/brain».
...
- **Дефолт автора для Plaud** — если источник метаданных не определяется, автор по умолчанию `unknown`, но для записей/конспектов Plaud (маркеры «note 1/1», «auto_sum_note», «consumer_note», «Meeting Minutes») — `plaud`

## Связанные скиллы

- [`brain-vector`](https://github.com/vokasug/hermes-skill-brain-vector) — векторная база поверх `*_sum.md`: семантический поиск по вопросам «мозг …» с фильтрами по папке/автору/дате. Пополнение — кроном каждые 4 часа (инкрементально), оптимизация — ежедневно в 06:00. Новый sum-файл из этого скилла попадёт в базу ближайшим прогоном.
