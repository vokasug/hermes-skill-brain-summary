...
1. **«Добавь в мозг» / «добавь в brain»** + что добавить — архивация любого контента в базу знаний `~/gdrive/Brain/`: медиа (тот же пайплайн yt-dlp + mlx-whisper), статьи и страницы (tavily extract), темы без ссылки (поиск → лучший источник), файлы (аудио/видео → транскрибация; документы/html → трансформация в MD, для html — `scripts/html2md.py`), готовый текст.
